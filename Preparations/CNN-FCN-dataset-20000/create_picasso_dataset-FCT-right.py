...
PICASSO_ROOT = os.path.join(DATASET_ROOT, 'picasso_dataset/FCT-data/right')   #Change: added Features Closer Together data folder
...
for dc in data_chunks: #go over all images
    ...
    lower_row = min_row + 16                                           #Change: added
    righter_col = min_col + 8                                            #Change: added
    left_eye_orig_positions.append(tuple((lower_row, righter_col)))     #Change: instead of min_row, min_col
    ...
    min_col = np.min(indices[1])
    lower_row = min_row + 16                                          #Change: added
    lefter_col = min_col - 8                                          #Change: added
    right_eye_orig_positions.append(tuple((lower_row, lefter_col)))   #Change: instead of min_row, min_col
    for i in range(len(indices[0])):
        indices[0][i] -= min_row
    for i in range(len(indices[1])):
        indices[1][i] -= min_col
    ...
    min_row = np.min(indices[0])
    min_col = np.min(indices[1])
    higher_row = min_row - 12                                            #Change: added
    mouth_orig_positions.append(tuple((higher_row, min_col)))            #Change: instead of min_row, min_col
    for i in range(len(indices[0])):
        indices[0][i] -= min_row
    for i in range(len(indices[1])):
        indices[1][i] -= min_col
