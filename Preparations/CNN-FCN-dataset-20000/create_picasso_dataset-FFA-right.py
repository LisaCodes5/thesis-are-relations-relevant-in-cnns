...
Original Authors: Johannes Rabold and Gesina Schwalbe
Changes by Lisa Lengenfelder
"""
...
PICASSO_ROOT = os.path.join(DATASET_ROOT, 'picasso_dataset/FFA-data/right')   #Change: added Features Further Apart data folder
"""Path to the root directory into which to save the generated images."""

#Set DataSet size
DATASET_SIZE = 20000                                                    #Change
CREATE_MASKS = True

origs_files = listdir(ORIGS_PATH)
NUM_ORIGS = len(origs_files)
no_faces_files = listdir(NO_FACES_PATH)
NUM_CANVASES = len(no_faces_files)
...
for dc in data_chunks: #go over all images
    ...
    higher_row = min_row - 20                                           #Change: added
    lefter_col = min_col - 10                                           #Change: added
    left_eye_orig_positions.append(tuple((higher_row, lefter_col)))     #Change: instead of min_row, min_col
    ...
    min_col = np.min(indices[1])
    higher_row = min_row - 20                                           #Change: added
    righter_col = min_col + 5                                           #Change: added
    right_eye_orig_positions.append(tuple((higher_row, righter_col)))   #Change: instead of min_row, min_col
    for i in range(len(indices[0])):
        indices[0][i] -= min_row
    for i in range(len(indices[1])):
        indices[1][i] -= min_col
    ...
    min_row = np.min(indices[0])
    min_col = np.min(indices[1])
    lower_row = min_row + 20                                            #Change: added
    mouth_orig_positions.append(tuple((lower_row, min_col)))            #Change: instead of min_row, min_col
    for i in range(len(indices[0])):
        indices[0][i] -= min_row
    for i in range(len(indices[1])):
        indices[1][i] -= min_col
...
def generate_image(canvas: int, organ_mapping: Sequence[Sequence[str]]) -> np.ndarray:
    ...
    for om in organ_mapping:
        ...
        while flag:
            try:
                flag = False
                organ_donor: int = np.random.randint(NUM_ORIGS)
        ...
        offset = bigger - smaller												#Change: move whole picture to the right
...
for i in tqdm(range(DATASET_SIZE//2)):
    ...
    canvas = np.random.randint(NUM_CANVASES)
    gi, gm = generate_image(canvas, [['left_eye', 'left_eye'], ['right_eye', 'right_eye'], ['nose', 'nose'], ['mouth', 'mouth']])
    gi = gi.astype(float)
    gi /= 255.0
    if i <= int(DATASET_SIZE // 2 * 0.8):
        split = "train"
    else:
        if i <= int(DATASET_SIZE // 2 * 0.95):
            split = "test"
        else:
            split = "valid"                                             #Change: added validation splitting
